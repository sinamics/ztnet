...
- **Endpoint URL (HTTPS)**: This is the receiver URL where ZTNET will send event data. It must be an HTTPS URL, and by default a publicly accessible one. See the note below for receivers on a private network.
