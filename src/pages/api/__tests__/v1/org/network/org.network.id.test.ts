...
import { GET_network } from "~/pages/api/v1/org/[orgid]/network/[nwid]";
...
describe("organization networkid api validation", () => {
	...
	test('GET_network should throw "Invalid Token"', async () => {
		...
		await GET_network(mockRequest as NextApiRequest, mockResponse as NextApiResponse);
		expect(mockResponse.status).toHaveBeenCalledWith(401);
		// check if the response is correct. invalid token
		expect(mockResponse.json).toHaveBeenCalledWith({ error: "Invalid token" });
	});

	test('GET_network should throw "Invalid Authorization Type" for mismatched authorization types', async () => {
