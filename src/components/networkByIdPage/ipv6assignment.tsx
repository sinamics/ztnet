...
export const Ipv6assignment = ({ central = false, organizationId }: IProp) => {
	...
	return (
		<div className="w-full space-y-4">
			...
			{/* IPv6 IP Assignment Pools - Vertical List */}
				...
				<div className="space-y-4">
					<div className="space-y-2">
						{network?.ipAssignmentPools
							?.filter(
								(pool) =>
									!Address4.isValid(pool.ipRangeStart) ||
									!Address4.isValid(pool.ipRangeEnd),
							)
							?.map((pool, index) => (
								<div
									// biome-ignore lint/suspicious/noArrayIndexKey: <explanation>
									key={index}
									className="badge badge-primary badge-lg flex w-full min-w-fit justify-between rounded-md p-3"
								>
									...
									<div title={t("networkIpAssignments.ipv4.delete_ip_assignment")}>
										<svg
											...
											strokeWidth="1.5"
											stroke="currentColor"
											className="h-4 w-4 cursor-pointer text-warning hover:text-red-500"
						...
						<div className="form-control w-full">
							...
							<div className="join w-full">
								<input
									...
									placeholder="2001:abcd:ef00:ffff:ffff:ffff:ffff:ffff"
								/>
							</div>
						</div>
